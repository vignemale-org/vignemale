...
| `call` | **appels service-à-service** : directs en local, **HTTP signé (HMAC)** une fois déployé — même code ; propagation de l'**auth** et du **trace-id W3C** entre services | `examples/shop/`, `tests/test_call.py` |
| `sqldb` | **Postgres** (pool de connexions, requêtes `$1`/`$2`, lignes → dicts) | `SQLDatabase`, `examples/todo.py` |
...
**Appels service-à-service** (le multi-service devient réel) :

```python
@api(method="POST", path="/orders")
def create_order(body: Order) -> dict:
    item = call("catalog", "get_item", id=body.item_id)   # ← même code partout
    ...
```
