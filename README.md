...
├── cli/                           ⚙️ la CLI (package séparé `vignemale-cli`)
│   └── vignemale_cli/             run · check · gen · rgpd · collect (parser) · devinfra
└── runtimes/
    ├── core/                      cœur Rust
    │   ├── build.rs               prost_build → types Rust
    │   └── src/                   config.rs · api/ · sqldb/ · secrets/ · objects.rs · …
    └── python/                    binding PyO3 + SDK (= ce qui part en PROD)
        ├── src/lib.rs             expose le core (module `_core`)
        ├── python/vignemale/      le package SDK — seule dépendance : pydantic
        └── tests/                 la suite pytest (SDK + CLI)
```

> Séparation façon Encore : le **runtime** (`vignemale`) est ce que les apps
> embarquent en production (dépendance unique : pydantic) ; la **CLI**
> (`vignemale-cli`, qui dépend du runtime) porte l'outillage développeur —
> parser statique, provisioning local docker, génération de clients, RGPD.
...
# 3) lancer les tests
uv pip install pytest && uv pip install -e ../../cli   # 1re fois seulement
